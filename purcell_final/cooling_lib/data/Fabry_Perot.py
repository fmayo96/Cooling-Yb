...
def cavity_spont_emi_rate(str_,
    ...
    if transition_linewidth == True:
        ...
        mask = (lambda_ > 971) & (lambda_ < 972.5)
        ...
        ax.grid(alpha = 0.2)
        ax.scatter(T[:6], ECS_centers[:6], color='C3', s = 10, label = 'Central wavelength', zorder = 3)
        ax.tick_params(axis='y')
        ax.errorbar(T[:6], ECS_centers[:6], yerr=ECS_widths[:6], alpha = 0.8, lw = 1,
        ...
        # Banda de la cavidad
        h_0 = center_fit_gamma - width_fit_gamma
        h_1 = center_fit_gamma
        h_2 = center_fit_gamma + width_fit_gamma
        ax.axhline(y=h_1, color='C2', linestyle='-', linewidth=0.5,
                   label ='Cavity resonance')
        v_min, v_max = -0.1, 1.1
        T_fill = np.array([50, 220])
    ...
    return
#%%
cavities_ = ['Reasonable cavity R_90','Reasonable cavity R_95',
...
cavity_parameters_b = []
cavity_inhibition_lw = np.zeros(len(cavities_[:8]))
cavity_enhancement_lw = np.zeros(len(cavities_[:8]))
cavity_inhibition_b = np.zeros(len(cavities_[8:]))
cavity_enhancement_b = np.zeros(len(cavities_[8:]))
j = 0
for i in range(len(cavities_)):
...
def ini_and_enh_behaviour(str_):
    if str_ == 'Mirror reflectance':
        ...
        line2 = ax2.scatter(cavity_parameters_lw[:,2],
                    ...
                    s = 30, marker = 'D', color = 'C2',
                    label = 'Max Inhibition')
        ax2.set_ylabel('Maximum Inhibition', color='C2')
